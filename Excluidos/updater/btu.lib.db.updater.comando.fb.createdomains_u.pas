...
uses btu.lib.db.updater.comando, System.Classes, btu.lib.db.updater.campo.list,
  ...
  sis.ui.io.ProcessLog, sis.ui.io.output, btu.lib.lists.TextoList;
...
//  oDBQuery.Prepare;
  try
    ...
    begin
      ...
      oDBQuery := DBQueryCreate(DBConnection, sSql, ProcessLog, Output);
...
begin
  ...
  oDBQuery := btu.lib.db.factory.DBQueryCreate(DBConnection, sSql, ProcessLog, Output);
  oDBQuery.Prepare;
  try
    for I := 0 to FComandosTextoList.Count - 1 do
    begin
