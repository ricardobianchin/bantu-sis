...
uses
  ...
  btu.lib.db.types, sis.UI.io.ProcessLog, sis.UI.io.output;
...
type
  ...
  public
    ...
    constructor Create(pDBConnection: IDBConnection; pSql: string; pProcessLog: IProcessLog;
      pOutput: IOutput);
...
begin
  Result := False;
  try
    ProcessLog.Exibir('Abrirá '+SQL+','+GetParamsAsStr);
    FFDQuery.Open;
    if FFDQuery.Active then
    begin
      Result := True;
      ProcessLog.Exibir('Abriu');
      UltimoErro := ''
    end;
  except
    on e: exception do
...
var
  s: string;
begin
  inherited Create(pDBConnection, pProcessLog, pOutput);
...
begin
  ProcessLog.Exibir('TDBQueryFireDac.Destroy');
  FreeAndNil(FFDQuery);
...
begin
  ProcessLog.Exibir('TDBQueryFireDac.Fechar,FFDQuery.Close');
  FFDQuery.Close;
end;
...
begin
  ...
  begin
    ProcessLog.Exibir('TDBQueryFireDac.Prepare,FFDQuery.Prepare');
    FFDQuery.Prepare;
  end;
end;
...
begin
  ...
  begin
    ProcessLog.Exibir('TDBQueryFireDac.Prepare,FFDQuery.Unprepare');
    FFDQuery.Unprepare;
  end;
end;
