...
uses
  ...
  Vcl.Controls, Vcl.Forms, Vcl.Dialogs, sis.ui.io.ProcessLog, Vcl.AppEvnts,    Sis.UI.IO.Output.Form_u,
...
uses Sta.exec_u, sis.ui.Img.DataModule, sis.ui.io.ProcessLog.factory,
...
begin
  try
  FProcessLog := ProcessLogFileCreate( 'Starter');

//  FOutputForm := TOutputForm.Create(self);
//  FOutputToForm := OutputToFormCreate(FOutputForm);
  FProcessLog.Exibir('TPrincForm.FormCreate, TStarterExec.Create');
  e := TStarterExec.Create(FProcessLog, Self, FOutputStatus);
  try
    e.Execute;
  finally
    e.Free;
    ...
    FProcessLog.Exibir('TPrincForm.ExecStarter Erro '+E.ClassName+' '+e.Message);
  end;
  FProcessLog.Exibir('Vai finalizar a aplicação');
