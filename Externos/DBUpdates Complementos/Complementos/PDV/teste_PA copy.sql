...
SELECT * FROM TESTE_PA.SESS_RELAT_FECH_TOTAIS_GET(1, 1, 1);
...
AS
BEGIN
  PROCEDURE EST_MOV_ITEM_VOLUMES_GET
  (
...
BEGIN
  ...
  BEGIN
    SELECT SUM(
      CASE 
        WHEN QTD = CAST(QTD AS INTEGER) THEN QTD 
        ELSE 1 
      END
    ) QTD_ITEM
    FROM EST_MOV_ITEM
  ...
  AS
    ...
    DECLARE POR_HORAQTD INTEGER;
    DECLARE POR_HORATOTAL DINH_DOM;
  ...
  BEGIN
    ...
    DO
    BEGIN
      LINHA = '4;' || :POR_HORA_HORA || ';' || :POR_HORAQTD || ';' || :POR_HORATOTAL;
      SUSPEND;
    END
  END
