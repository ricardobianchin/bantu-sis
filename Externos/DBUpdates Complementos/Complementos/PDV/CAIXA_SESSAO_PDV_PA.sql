...
*/
  ...
  PROCEDURE SESS_RELAT_VENDA_CANC_GET
  (
    ABE_LOG_ID BIGINT NOT NULL,
    FECH_LOG_ID BIGINT NOT NULL
  )
  RETURNS
  (
    LOJA_ID ID_SHORT_DOM NOT NULL
  ...
  PROCEDURE SESS_RELAT_VENDA_ITEM_CANC_GET
  (
    ABE_LOG_ID BIGINT NOT NULL,
    FECH_LOG_ID BIGINT NOT NULL
  )
...
*/  
  ...
  BEGIN
    ...
    FOR
      WITH SESS AS
    (
      SELECT LOJA_ID, TERMINAL_ID, SESS_ID, SESS_LOG_ID
      FROM CAIXA_SESSAO
      WHERE LOJA_ID = :SESS_LOJA_ID
      AND TERMINAL_ID = :SESS_TERMINAL_ID
    ...
    (
    SELECT
    O.LOJA_ID, O.TERMINAL_ID, O.SESS_ID, LOG.DTH
    FROM CAIXA_SESSAO_OPERACAO O
    JOIN LOG ON
    O.LOJA_ID = LOG.LOJA_ID
    AND O.TERMINAL_ID = LOG.TERMINAL_ID
    AND O.OPER_LOG_ID = LOG.LOG_ID
    ...
    SELECT
    SESS.SESS_ID, PES.APELIDO, INI.DTH ABERTO_EM, FIM.DTH FECHADO_EM
    FROM SESS

    JOIN L ON
    SESS.LOJA_ID = L.LOJA_ID
    AND SESS.TERMINAL_ID = L.TERMINAL_ID
    AND SESS.SESS_LOG_ID = L.LOG_ID

    JOIN PES ON
    PES.LOJA_ID = L.LOJA_ID
    AND PES.TERMINAL_ID = L.PESSOA_TERMINAL_ID
    AND PES.PESSOA_ID = L.PESSOA_ID

    JOIN INI ON 
    SESS.LOJA_ID = INI.LOJA_ID
    AND SESS.TERMINAL_ID = INI.TERMINAL_ID
    AND SESS.SESS_ID = INI.SESS_ID

    LEFT JOIN FIM ON 
    SESS.LOJA_ID = FIM.LOJA_ID
    AND SESS.TERMINAL_ID = FIM.TERMINAL_ID
    AND SESS.SESS_ID = FIM.SESS_ID
  INTO
    :SESS_ID
    , :OPER_APELIDO
    , :ABERTO_EM
  ...
  AS
    ...
    DECLARE ABE_LOG_ID BIGINT;
    DECLARE FECH_LOG_ID BIGINT;
    
    DECLARE CANC_VEN_LOJA_ID ID_SHORT_DOM;
    DECLARE CANC_VEN_TERMINAL_ID ID_SHORT_DOM;
    DECLARE CANC_VEN_VENDA_ID BIGINT;
    DECLARE CANC_VEN_CRIADO_EM TIMESTAMP;
    DECLARE CANC_VEN_CANCELADO_EM TIMESTAMP;
    
    DECLARE CANC_ITEM_LOJA_ID ID_SHORT_DOM;
    DECLARE CANC_ITEM_TERMINAL_ID ID_SHORT_DOM;
    DECLARE CANC_ITEM_VENDA_ID BIGINT;
    DECLARE CANC_ITEM_ORDEM SMALLINT;
    DECLARE CANC_ITEM_CANCELADO_EM TIMESTAMP;
  BEGIN
    -- INICIO DO RELATORIO INICIO
    ...
    SELECT ABE_LOG_ID, FECH_LOG_ID
    FROM CAIXA_SESSAO_INICIAL_PA.LOG_ABE_FECH_GET
      (:LOJA_ID, :TERMINAL_ID, :SESS_ID)
    ...
    FOR
      SELECT 
        ...
        , CRIADO_EM
        , CANCELADO_EM
      FROM SESS_RELAT_VENDA_CANC_GET(:ABE_LOG_ID, :FECH_LOG_ID)
    INTO
      :CANC_VEN_LOJA_ID
      , :CANC_VEN_TERMINAL_ID
      , :CANC_VEN_VENDA_ID
      , :CANC_VEN_CRIADO_EM
      , :CANC_VEN_CANCELADO_EM
    DO
    BEGIN
      LINHA = '5;' 
        || :CANC_VEN_LOJA_ID
    ...
    FOR
      SELECT 
        ...
        , ORDEM
        , CANCELADO_EM
      FROM SESS_RELAT_VENDA_ITEM_CANC_GET(:ABE_LOG_ID, :FECH_LOG_ID)
    INTO
      :CANC_ITEM_LOJA_ID
      , :CANC_ITEM_TERMINAL_ID
      , :CANC_ITEM_VENDA_ID
      , :CANC_ITEM_ORDEM
      , :CANC_ITEM_CANCELADO_EM
    DO
    BEGIN
      LINHA = '6;' 
        || :CANC_ITEM_LOJA_ID
  ...
  BEGIN
    FOR
      SELECT
        ...
        EM.CANCELADO_EM
      FROM LOG L
      JOIN LOG_ENVOLVE_ID E ON
        L.LOJA_ID = E.LOJA_ID
        AND L.TERMINAL_ID = E.TERMINAL_ID
        AND L.LOG_ID = E.LOG_ID
      JOIN EST_MOV EM ON
        EM.LOJA_ID = E.LOJA_ID_ENVOLVIDO
        AND EM.TERMINAL_ID = E.TERMINAL_ID_ENVOLVIDO
        AND EM.EST_MOV_ID = E.ID_ENVOLVIDO
      JOIN VENDA V ON 
        V.LOJA_ID = E.LOJA_ID_ENVOLVIDO
        AND V.TERMINAL_ID = E.TERMINAL_ID_ENVOLVIDO
        AND V.EST_MOV_ID = E.ID_ENVOLVIDO
      WHERE
        L.FEATURE_SIS_ID = 17
        AND L.ACAO_SIS_ID=')'
        AND L.LOG_ID > :ABE_LOG_ID
        AND ((:FECH_LOG_ID = 0) OR (L.LOG_ID < :FECH_LOG_ID))
      ORDER BY 
        V.LOJA_ID,
        V.TERMINAL_ID,
        V.VENDA_ID
  ...
  BEGIN
    FOR
      ...
      JOIN EST_MOV_ITEM EMI ON
        V.LOJA_ID = EMI.LOJA_ID
        AND V.TERMINAL_ID = EMI.TERMINAL_ID
        AND V.EST_MOV_ID = EMI.EST_MOV_ID
        AND E.ORDEM_ENVOLVIDO = EMI.ORDEM
      WHERE
        L.FEATURE_SIS_ID = 18
        AND L.ACAO_SIS_ID=')'
        AND L.LOG_ID > :ABE_LOG_ID
...
SELECT * FROM CAIXA_SESSAO_PDV_PA.SESS_RELAT_BYID_GET(1, 3, 1);
SELECT * FROM CAIXA_SESSAO_PDV_PA.SESS_RELAT_BYID_GET(1, 3, 2);

SELECT * FROM CAIXA_SESSAO_PDV_PA.SESS_RELAT_VENDA_CANC_GET(2, 23);
SELECT * FROM CAIXA_SESSAO_PDV_PA.SESS_RELAT_VENDA_CANC_GET(24,0);

SELECT * FROM CAIXA_SESSAO_PDV_PA.SESS_RELAT_VENDA_ITEM_CANC_GET(2, 23);
SELECT * FROM CAIXA_SESSAO_PDV_PA.SESS_RELAT_VENDA_ITEM_CANC_GET(24,0);
