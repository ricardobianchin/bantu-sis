/*
in "C:\Pr\app\bantu\bantu-sis\Src\Externos\DBUpdates Complementos\Complementos\Sis\LOG_HIST_PA.sql";
SELECT * FROM LOG_HIST_PA.TEVE_DESPESA_TIPO(0, 1000);
...
BEGIN
  ...
  BEGIN
    SELECT DISTINCT
      ...
      , T.BALANCA_MODO_USO_ID
      , T.BALANCA_ID

      , T.BARRAS_COD_INI
      , T.BARRAS_COD_TAM

      , T.IMPRESSORA_MODO_ENVIO_ID
      , T.IMPRESSORA_MODELO_ID
      , T.IMPRESSORA_NOME
      , T.IMPRESSORA_COLS_QTD

      , T.CUPOM_QTD_LINS_FINAL
      , T.SEMPRE_OFFLINE
      , T.ATIVO
    ...
    JOIN LOG_ENVOLVE_ID LOG_EN ON
      ...
      AND LOG_EN.TERMINAL_ID = LOG.TERMINAL_ID
      AND LOG_EN.LOG_ID = LOG.LOG_ID
      AND LOG_EN.TERMINAL_ID_ENVOLVIDO = :P_TERMINAL_ID

    JOIN TERMINAL t ON
      T.TERMINAL_ID = :P_TERMINAL_ID
    INTO
      :TERMINAL_ID
  ...
  */
  ...
  BEGIN
    ...
    JOIN LOG ON 
      AMBI.LOJA_ID = LOG.LOJA_ID
      AND LOG.TERMINAL_ID = 0 -- TERM RETAGUARDA
      AND FEATURE_SIS_ID = 16 -- FEATURE DESPESA_TIPO
      AND LOG_ID >= :LOG_ID_INI
      AND LOG_ID <= :LOG_ID_FIN
    ...
    JOIN DESPESA_TIPO D ON
      LOG_EN.ID_ENVOLVIDO = D.DESPESA_TIPO_ID
