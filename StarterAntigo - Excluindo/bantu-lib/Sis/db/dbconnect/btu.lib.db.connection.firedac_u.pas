...
uses btu.lib.db.connection_u, FireDAC.Stan.Intf,
  ...
  FireDAC.Stan.Def, FireDAC.Phys, FireDAC.Comp.Client, sis.UI.io.ProcessLog,
...
begin
  ...
  ProcessLog.Exibir('TDBConnectionFireDAC.Commit,FFDConnection.Commit');
...
var
  ...
  s: string;
begin
  inherited Create(pDBConnectionParams, pProcessLog, pOutput);
  ...
  ProcessLog.Exibir('TDBConnectionFireDAC.Create'#13#10 +
...
begin
  inherited;
  FFDConnection.Close;
  ProcessLog.Exibir('Desconectou,' + FFDConnection.Params.CommaText);
end;
...
begin
  ...
  ProcessLog.Exibir('TDBConnectionFireDAC.GetValue,vai Abrir');
  Abrir;
  try
    ...
    ProcessLog.Exibir('TDBConnectionFireDAC.GetValue,vai Fechar');
    Fechar;
  end;
end;
...
begin
  ProcessLog.Exibir('FFDConnection.Rollback');
  FFDConnection.Rollback;
end;

procedure TDBConnectionFireDAC.StartTransaction;
begin
  ProcessLog.Exibir('FFDConnection.StartTransaction');
