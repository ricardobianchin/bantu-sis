...
uses btu.lib.db.types, btu.lib.db.dbms, btu.lib.config, sis.ui.io.ProcessLog,
...
function DBConnectionCreate(pSisConfig: ISisConfig; pDBMS: IDBMS;
  pLocalDoDB: TLocalDoDB; pProcessLog: IProcessLog; pOutput: IOutput): IDBConnection;

function DBExecCreate(pDBConnection: IDBConnection; pSql: string; pProcessLog: IProcessLog;
      pOutput: IOutput): IDBExec;

function DBQueryCreate(pDBConnection: IDBConnection; pSql: string; pProcessLog: IProcessLog;
...
function DBConnectionCreate(pSisConfig: ISisConfig; pDBMS: IDBMS; pLocalDoDB: TLocalDoDB; pProcessLog: IProcessLog; pOutput: IOutput): IDBConnection;
begin
  case pSisConfig.DBMSInfo.DBFramework of
    dbfrFireDAC:
    begin
      Result := TDBConnectionFiredac.Create(pSisConfig.DBMSInfo, pDBMS.LocalDoDBToConnectionParams(pLocalDoDB), pProcessLog, pOutput);
    end;
  end;
end;
...
function DBExecCreate(pDBConnection: IDBConnection; pSql: string; pProcessLog: IProcessLog;
      pOutput: IOutput): IDBExec;
begin
  Result := TDBExecFireDac.Create(pDBConnection, pSql, pProcessLog, pOutput);
end;

function DBQueryCreate(pDBConnection: IDBConnection; pSql: string; pProcessLog: IProcessLog;
      pOutput: IOutput): IDBQuery;
begin
  Result := TDBQueryFireDac.Create(pDBConnection, pSql, pProcessLog, pOutput);
end;
