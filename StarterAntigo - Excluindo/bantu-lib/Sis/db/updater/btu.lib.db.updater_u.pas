...
uses
  btu.lib.db.updater, btu.lib.config, sis.ui.io.ProcessLog, sis.ui.io.output,
...
begin
  ...
  FProcessLog.Exibir('TDBUpdater.Create FsLocalDoDB=' + FsLocalDoDB +
    ',FCaminhoComandos=' + FCaminhoComandos);
  FDBConnection := DBConnectionCreate(FSisConfig, FDBMS, FLocalDoDB,
    FProcessLog, FOutput);

  FDBUpdaterOperations := GetDBUpdaterOperations(FDBConnection, FProcessLog, FOutput);

  FComandoList := ComandoListCreate;

  FProcessLog.Exibir('TDBUpdater.Create fim');
end;
...
begin
  Result := 0;
  FProcessLog.Exibir('TDBUpdater.DBDescubraVersaoEConecte inicio');
  try
    FProcessLog.Exibir('TDBUpdater.DBDescubraVersaoEConecte, vai testar se o banco existe');
    if not GetDBExiste then
    begin
      FProcessLog.Exibir('TDBUpdater.DBDescubraVersaoEConecte,banco nao existia');
      CrieDB;
      if not GetDBExiste then
      begin
        sErro := 'TDBUpdater.DBDescubraVersaoEConecte,Erro ao criar banco de dados';
        FProcessLog.Exibir(sErro);
        raise Exception.Create(sErro);
      end;
    end
    else
    FProcessLog.Exibir('TDBUpdater.DBDescubraVersaoEConecte,banco existia,vai abrir conexao');

    DBConnection.Abrir;
    FDBUpdaterOperations.PreparePrincipais;
    FProcessLog.Exibir('TDBUpdater.DBDescubraVersaoEConecte,vai testar se tabela ' + NOMETAB_DBUPDATE_HIST + ' existe');
    if not FDBUpdaterOperations.TabelaExiste(NOMETAB_DBUPDATE_HIST) then
    begin
      FProcessLog.Exibir('TDBUpdater.DBDescubraVersaoEConecte,nao existia,vr=-1');
      Result := -1;
      exit;
    end;
    FProcessLog.Exibir('TDBUpdater.DBDescubraVersaoEConecte, existia,vai ler com versao_get');

    Result := FDBUpdaterOperations.VersaoGet;
    FProcessLog.Exibir('TDBUpdater.DBDescubraVersaoEConecte, FDBUpdaterOperations.VersaoGet=' + Result.ToString);
  finally
    FProcessLog.Exibir('TDBUpdater.DBDescubraVersaoEConecte fim');
  end;
end;

destructor TDBUpdater.Destroy;
begin
  FProcessLog.Exibir('TDBUpdater.Destroy, FsLocalDoDB=' + FsLocalDoDB);
  FDestinoSL.Free;
  inherited;
end;
    ...
    try
      ...
      try
        repeat
          ...
          FProcessLog.Exibir('TDBUpdater.Execute,retornou True');

          FDtHExec := Now;
          RemoveExcedentes(FLinhasSL);
          LerUpdateProperties(FLinhasSL);

          FProcessLog.Exibir('TDBUpdater.Execute,vai ComandosCarregar');
          ComandosCarregar;

          FProcessLog.Exibir('TDBUpdater.Execute,vai ComandosGetSql');
          ComandosGetSql;

          FProcessLog.Exibir('TDBUpdater.Execute,vai ExecuteSql');
          ExecuteSql;

          FProcessLog.Exibir('TDBUpdater.Execute,vai ComandosTesteFuncionou');
          ComandosTesteFuncionou;

          FProcessLog.Exibir('TDBUpdater.Execute,vai GravarVersao');
          GravarVersao;

        until False;
      finally
        FProcessLog.Exibir('TDBUpdater.Execute,vai FDBUpdaterOperations.Unprepare');
      ...
      begin
        ...
        FProcessLog.Exibir('TDBUpdater.Execute,erro vr. ' + iVersao.ToString + E.Message);
        // DBConnection.Rollback;
      end;
    end;
  finally
    FProcessLog.Exibir('TDBUpdater.Execute,fechando DBConnection e saindo');
...
var
  ...
  sTipoComando: string;
begin
  FProcessLog.Exibir('TDBUpdater.ComandosCarregar,ini;');
  ...
  try
    ...
    FProcessLog.Exibir('TDBUpdater.ComandosCarregar,fim;');
  end;
end;
...
begin
  ...
  dbms.ExecInterative('DBUpdate ' + IntToStrZero(iVersao, 9), FDestinoSL.Text,
    FLocalDoDB, FProcessLog, FOutput);
end;
