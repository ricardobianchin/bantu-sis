...
uses btu.lib.db.updater.operations, btu.lib.db.types, sis.ui.io.ProcessLog,
...
begin
  ...
  FDBQueryTabelaExiste := DBQueryCreate(pDBConnection, s, pProcessLog, pOutput);

  s := GetSQLIndexNamesParams;
  FDBQueryIndexNames := DBQueryCreate(pDBConnection, s, pProcessLog, pOutput);

  s := GetSQLProcedureExisteParams;
  FDBQueryProcedureExiste := DBQueryCreate(pDBConnection, s, pProcessLog, pOutput);

  s := GetSQLPackagGetCodigoParams;
  FDBQueryPackageGetCodigo := DBQueryCreate(pDBConnection, s, pProcessLog, pOutput);

  s := GetSQLVersaoGet;
  FDBQueryVersaoGet := DBQueryCreate(pDBConnection, s, pProcessLog, pOutput);

  s := GetSQLHistInsParams;
  FDBExecHistIns := DBExecCreate(pDBConnection, s, pProcessLog, pOutput);
end;
...
begin
  ...
  FDBQueryForeignKeyInfo := DBQueryCreate(FDBConnection, s, FProcessLog, FOutput);
...
begin
  ...
  FDBQuerySequenceExiste := DBQueryCreate(FDBConnection, s, FProcessLog, FOutput);
